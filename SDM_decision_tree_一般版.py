...
K = 10
kfp = KFoldClassificationPerformance(x_ary = X, y_ary = Y, classifier = classifier, k_fold = K)
...
micro_F1 = cross_val_score(estimator = classifier, X = X, y = Y,scoring = 'f1_micro', cv = K, n_jobs = 1)
...
AUC = cross_val_score(estimator = classifier, X = X, y = Y,scoring = 'roc_auc', cv = K, n_jobs = 1)
...
# 生成點陣圖
graph = md.tree_drawer(classifier = classifier, feature_names = X_test.columns, target_names = cls_name, graphviz_bin = GRAPHVIZ_INSTALL)
# 點陣圖轉換為 PNG
...
# dot_data = tree.export_graphviz(classifier, filled = True, feature_names = X_test.columns, class_names = cls_name, rounded = True, special_characters = True)
